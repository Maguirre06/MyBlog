...
namespace Myblog.Core.Controllers
{
    public class BlogController : Controller
    {
        ...
        [Route("/find/{page:int?}")]
        [OutputCache(Profile = "default")]
        public async Task<IActionResult> IndexB([FromRoute]int page = 0)
        {
            var posts = await _blog.GetPosts(_settings.Value.PostsPerPage, _settings.Value.PostsPerPage * page);
            ViewData["Title"] = _manifest.Name;
            ViewData["Description"] = _manifest.Description;
            ViewData["prev"] = $"/{page + 1}/";
            ViewData["next"] = $"/{(page <= 1 ? null : page - 1 + "/")}";
            return View("~/Views/Blog/IndexB.cshtml", posts);
        }
        ...
        // This is for redirecting potential existing URLs from the old Myblog URL format
        [Route("/post/{slug}")]
        [HttpGet]
